...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		findElements: function() {
			var self = this;
			self.el = self.el || {};
			self.el.search = self.box.querySelector(classSelector('indexed-list-search'));//输入的检索值div
			self.el.searchInput = self.box.querySelector(classSelector('indexed-list-search-input'));//获取输入的检索内容
		...
		search: function(keyword) {
			...
			liArray.forEach(function(item) {
				...
				if (item.classList.contains($.className('indexed-list-group'))) {
					...
					var value1 = (item.getAttribute('data-1') || '').toLowerCase();
					var value2 = (item.getAttribute('data-2') || '').toLowerCase();
					var value3 = (item.getAttribute('data-3') || '').toLowerCase();
					var value4 = (item.getAttribute('data-4') || '').toLowerCase();
					var value5 = (item.getAttribute('data-5') || '').toLowerCase();
					var value6 = (item.getAttribute('data-6') || '').toLowerCase();
					var value7 = (item.getAttribute('data-7') || '').toLowerCase();
					if (keyword && text.indexOf(keyword) < 0 &&
						value.indexOf(keyword) < 0 &&
						tags.indexOf(keyword) < 0 &&
						value1.indexOf(keyword) < 0 &&
						value2.indexOf(keyword) < 0 &&
						value3.indexOf(keyword) < 0 &&
						value4.indexOf(keyword) < 0 &&
						value5.indexOf(keyword) < 0 &&
						value6.indexOf(keyword) < 0 &&
						value7.indexOf(keyword) < 0
						) {
